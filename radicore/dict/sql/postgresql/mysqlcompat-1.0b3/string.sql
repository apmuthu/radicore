...
CREATE OR REPLACE FUNCTION concat(text, text, text, text)
RETURNS text AS $$
  SELECT $1 || $2 || $3 || $4
$$ IMMUTABLE STRICT LANGUAGE SQL;

CREATE OR REPLACE FUNCTION concat(text, text, text, text, text)
RETURNS text AS $$
  SELECT $1 || $2 || $3 || $4 || $5
$$ IMMUTABLE STRICT LANGUAGE SQL;
...
RETURNS text AS $$
  SELECT CASE
    WHEN $1 IS NULL THEN NULL
    ELSE 
      coalesce($2 || $1, '') || coalesce($3 || $1, '') || coalesce($4, '')
  END
$$ IMMUTABLE LANGUAGE SQL;

CREATE OR REPLACE FUNCTION concat_ws(text, text, text, text, text)
RETURNS text AS $$
  SELECT CASE
    WHEN $1 IS NULL THEN NULL
    ELSE 
      coalesce($2 || $1, '') || coalesce($3 || $1, '') || coalesce($4, '') || coalesce($5, '')
  END
$$ IMMUTABLE LANGUAGE SQL;
