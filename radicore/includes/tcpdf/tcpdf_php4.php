...
if (!class_exists('TCPDF')) {
	 ...
	 */
	define('PDF_PRODUCER', 'TCPDF 5.8.013_PHP4 (http://www.tcpdf.org)');
	...
	*/
	class TCPDF {
		 ...
		 * @since 4.5.011
		 */
		function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellMargin='', $lineWidth='') {
			if ($this->empty_string($w) OR ($w <= 0)) {
				if ($this->rtl) {
					$w = $this->x - $this->lMargin;
				} else {
		 ...
		 * @access public
		 */
		function getStringHeight($w, $txt, $reseth=false, $autopadding=true, $cellMargin='', $lineWidth='') {
		 ...
		 * @since 1.5
		 */
		function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0) {
			if (strlen($txt) == 0) {
				$txt = ' ';
			}
		...
		function _putfontwidths($font, $cidoffset=0) {
			...
			foreach ($range as $k => $ws) {
				...
				if (isset($ws['interval'])) {
					...
					if (isset($range[$k]['interval'])) {  // AJM
						unset($range[$k]['interval']);    // AJM
					} // if                               // AJM
					--$nextk;
				} else {
					$prevint = false;
