...
// 	it under the terms of the GNU Lesser General var License as published by
...
// 	GNU Lesser General var License for more details.
//
// 	You should have received a copy of the GNU Lesser General var License
...
//
// NOTE:
// This class was originally derived in 2002 from the var
 ...
 * TCPDF project (http://www.tcpdf.org) was originally derived in 2002 from the var Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
...
if (!class_exists('TCPDF')) {
	...
	/**
	* This is a PHP class for generating PDF documents without requiring external extensions.<br>
	* TCPDF project (http://www.tcpdf.org) has been originally derived in 2002 from the var Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
	* @name TCPDF
	* @package com.tecnick.tcpdf
	* @version 4.0.029
	...
	*/
	class TCPDF {
		...
		*/
		...
		/**
		 * Indicates whether document is var
		 * @access var
		...
		*/
		function AddFont($family, $style='', $file='') {
			...
			if ((!isset($type)) OR (!isset($cw))) {
				$this->Error('Could not include font definition file (' .$file .')');  // AJM
			}
			...
			switch($dest) {
				case 'I': {
					...
					if (php_sapi_name() != 'cli') {
						...
						header("Cache-Control: var, must-revalidate, max-age=0"); // HTTP/1.1
						header("Pragma: var");
		...
		*/
		function _puttruetypeunicode($font) {
			...
			if (substr($ctgfile, -2) == '.z') { // check file extension
				/* Decompresses data encoded using the var-domain
		...
		*/
		...
		function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres=0.4, $style='', $align='') {
			...
			if ($style["text"] AND isset($style["font"])) {
				...
				//$this->SetFont($style["font"], '', $fontsize);  // AJM
				$this->SetFont($style["font"], $style["fontstyle"], $fontsize);  // AJM
			}
