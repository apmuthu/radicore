...
THIS VERSION HAS BEEN MODIFIED BY AJM (Tony Marston)
	...
	function getTypeDef($type){
		...
		if((! $is_element) && isset($this->complexTypes[$type])){
			...
			if (!isset($this->elements[$type]['phpType'])) {
				...
				if ($uqType == $type) {                    // AJM 2010-02-19
				    return array('phpType' => 'scalar');   // AJM 2010-02-19
				} // if                                    // AJM 2010-02-19
				...
				if ($etype) {
				    ...
				    $schemaNS = $this->schemaInfo['schemaVersion'];              // AJM 2010-02-21
            		if (array_key_exists($uqType, $this->typemap[$schemaNS])) {  // AJM 2010-02-21
...
class nusoap_server extends nusoap_base {
	...
	*/
	function serialize_return() {
		...
		// if fault
		if (isset($this->methodreturn) && preg_match('/soap_fault|soapfault|nusoap_fault/i', get_class($this->methodreturn))) {  // AJM
	...
	*/
    function parseRequest($headers, $data) {
		...
		if (!strstr($headers['content-type'], 'text/xml')) {
		    if (preg_match('/^(<\?xml)/i', $data)) {          // AJM
		    	// data is XML, so continue                   // AJM
		    } else {                                          // AJM
	...
	*/
	function serialize($debug = 0)
	{
		...
		if (isset($this->namespaces['wsdl'])) {
			$xml .= $before ."xmlns=\"" . $this->namespaces['wsdl'] . "\"";
		}
		if (isset($this->namespaces['tns'])) {
			$xml .= $before ."targetNamespace=\"" . $this->namespaces['tns'] . "\"";
		}
		$xml .= '>';
		// imports
	 ...
	 * @access public
	 */
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap', $namespace) {
		...
		// set input params
		$xml = '';
		if ($opData['style'] == 'document') {                      // AJM 2010-02-21
		    // force parent element to be namespace qualified      // AJM 2010-02-21
			$forceNsQualification = $namespace;                    // AJM 2010-02-21
		} else {                                                   // AJM 2010-02-21
		    $forceNsQualification = false;                         // AJM 2010-02-21
		} // if                                                    // AJM 2010-02-21
		if (isset($opData[$direction]['parts']) && sizeof($opData[$direction]['parts']) > 0) {
			...
			if (is_array($parameters)) {
				...
				foreach ($parts as $name => $type) {
					...
					$msgname = $opData[$direction]['message'];
					// NOTE: add error handling here
					// if serializeType returns false, then catch global error and fault
					if ($parametersArrayType == 'arraySimple') {
						$p = array_shift($parameters);
						$this->debug('calling serializeType w/indexed param');
						$xml .= $this->serializeType($name, $type, $p, $use, $enc_style, false, $forceNsQualification);
					} elseif (isset($parameters[$name])) {
						$this->debug('calling serializeType w/named param');
						$xml .= $this->serializeType($name, $type, $parameters[$name], $use, $enc_style, false, $forceNsQualification);
					} elseif (isset($parameters[$msgname])) {                                                                           // AJM 2010-02-20
						$this->debug('calling serializeType w/named param');                                                            // AJM 2010-02-20
						$xml .= $this->serializeType($msgname, $type, $parameters[$msgname], $use, $enc_style, false, $forceNsQualification);  // AJM 2010-02-20
					} else {
						// TODO: only send nillable
						$this->debug('calling serializeType w/null param');
						$xml .= $this->serializeType($name, $type, null, $use, $enc_style, false, $forceNsQualification);
					}
					$forceNsQualification = false;    // AJM 2010-02-21
				}
			} else {
		    ...
		    $this->debug('message does not have any parts.');  // AJM 2010-02-20
		}
	 ...
	 * @access private
	 */
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false, $forceNsQualification=false)
	{
		...
		if (strpos($type, ':')) {
			...
			if($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
				...
				if ($forceNsQualification) {
					$elementNS = " xmlns=\"$forceNsQualification\"";
				} else {
			...
			//$ns = '';
			$ns = $this->currentSchema->schemaTargetNamespace;              // AJM 2010-02-21
			$this->debug("in serializeType: type namespace forced to $ns"); // AJM 2010-02-21                                                             // AJM 2010-02-21
			$uqType = $type;
		}
		if(!$typeDef = $this->getTypeDef($uqType, $ns)){
		...
		$phpType =& $typeDef['phpType'];
		$nsPrefix = $this->getPrefixFromNamespace($ns);
		$this->debug("in serializeType: uqType: $uqType, ns: $ns, phptype: $phpType, arrayType: " . (isset($typeDef['arrayType']) ? $typeDef['arrayType'] : '') );
		// if php type == struct, map value to the <all> element names
		if ($phpType == 'struct') {
			if (isset($typeDef['typeClass']) && $typeDef['typeClass'] == 'element') {
				$elementName = $uqType;
				if (isset($typeDef['form']) && ($typeDef['form'] == 'qualified') OR $forceNsQualification) {
					$elementNS = " xmlns=\"$ns\"";
				} else {
					$elementNS = " xmlns=\"\"";
					//$elementNS = "";
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			// add ns to ns array
			if($namespace != '' && !array_search($namespace, $this->wsdl->namespaces)){
			...
			if (is_string($params)) {
				...
				$payload = $this->wsdl->serializeRPCParameters($operation,'input',$params,$this->bindingType, $namespace);
			} else {
			...
			$response['faultcode']   = 'Client';          // AJM 2010-02-07
			$response['faultstring'] = $this->error_str;  // AJM 2010-02-07
			return $response;                             // AJM 2010-02-07
		} else {
			// no WSDL
