...
// create object to read all available accounts
require_once 'classes/mnu_account.class.inc';
$account = new mnu_account;

unset($_SESSION['rdcaccount_id']);  // not restricted to a single account

// start with 'rdcaccount_id=NULL'
processAccount();

$accounts = $account->getData();
foreach ($accounts as $account_data) {
	processAccount($account_data['rdcaccount_id']);
} // foreach

batchEnd();

// ******************************************************************
function processAccount($account_id=null)
// process outstanding items which belong to this account
{
    global $workitem, $workflow;

    $now = getTimeStamp();
    if (empty($account_id)) {
    	$where = "workitem_status='EN' AND deadline <= '$now' AND rdcaccount_id IS NULL";
    } else {
        $where = "workitem_status='EN' AND deadline <= '$now' AND rdcaccount_id='$account_id'";
    } // if
